...
#define MAX_TRACE_DEPTH_IPV6 5
#define TRACE_SKIP_DEPTH_IPV6 2
struct refcnt_trace {
	int refcnt;
	unsigned int info;
	unsigned int entry_nr;
	unsigned long time;
	unsigned long entry[6];
};
...
static inline void dev_put(struct net_device *dev)
{
	...
	#if REFCNT_DEBUG
	if (!strncmp(dev->name, "ap0", 3)) {
		printk("[ap0] put:%s, refcnt=%d\n", dev->name, __this_cpu_read(*dev->pcpu_refcnt));
