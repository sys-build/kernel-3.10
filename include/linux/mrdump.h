...
/* HTC: move mrdump_cb to 0x42300000*/
#define MRDUMP_CB_ADDR (PHYS_OFFSET + 0x2300000)
...
static inline int mrdump_set_ramdump_reason(AEE_REBOOT_MODE reset_reson, char *msg)
{
        struct mrdump_control_block *cblock = NULL;

        cblock = (struct mrdump_control_block *)__va(MRDUMP_CB_ADDR);
        if (strncmp(cblock->sig, "MRDUMP01", 8) == 0){
            cblock->sig[0] = 'X';
        }
        return 0;
};
