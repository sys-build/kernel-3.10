...
static int mmc_blk_issue_discard_rq(struct mmc_queue *mq, struct request *req)
{
	...
	if (mmc_can_discard(card) && (card->cid.manfid != CID_MANFID_HYNIX))
		arg = MMC_DISCARD_ARG;
...
#define PRT_TIME_PERIOD	3000000000
...
static unsigned long long mmcqd_thread_t1[ID_CNT]={0};
static unsigned long long mmcqd_cpu_t1[ID_CNT]={0};
static unsigned long long mmcqd_cpu_t2[ID_CNT]={0};
static unsigned long long mmcqd_cpu_t3[ID_CNT]={0};
static unsigned int mmcqd_rw_rq_count[ID_CNT]={0};
static unsigned int mmcqd_discard_rq_count[ID_CNT]={0};
static unsigned int mmcqd_flush_rq_count[ID_CNT]={0};
...
static void g_var_clear(unsigned int idx)
{
				...
				mmcqd_thread_t1[idx]=0;
				mmcqd_cpu_t1[idx]=0;
				mmcqd_cpu_t2[idx]=0;
				mmcqd_cpu_t3[idx]=0;
				mmcqd_rw_rq_count[idx]=0;
				mmcqd_discard_rq_count[idx]=0;
				mmcqd_flush_rq_count[idx]=0;
}
		...
		reqs = mmc_blk_prep_packed_list(mq, rqc);
			...
			if(t_period >= (unsigned long long )PRT_TIME_PERIOD)
			{
				...
				if(t_period > t_usage*100) {
					pr_warn("[BLOCK_TAG] mmcqd:%d Workload < 1%%, duty %lld, total time %lld, period %lld, req_cnt=%d \n", mmcqd[idx], t_usage, mmcqd_thread_t1[idx], t_period, mmcqd_rq_count[idx]);
					pr_warn("[BLOCK_TAG] mmcqd:%d rw_time=%lld, discard_time=%lld, flush_time=%lld, rw_rq=%d, discard_rq=%d, flush_rq=%d\n", mmcqd[idx], mmcqd_cpu_t1[idx], mmcqd_cpu_t2[idx], mmcqd_cpu_t3[idx], mmcqd_rw_rq_count[idx], mmcqd_discard_rq_count[idx], mmcqd_flush_rq_count[idx]);
				}
				else
				{
					...
					t_percent =((unsigned int)mmcqd_thread_t1[idx])/((unsigned int)t_period);
					pr_warn("[BLOCK_TAG] mmcqd:%d Workload=%d%%, total load=%d%%, duty %lld, total time %lld, period %lld00, req_cnt=%d \n", mmcqd[idx], mmcqd_work_percent[idx], t_percent, t_usage, mmcqd_thread_t1[idx], t_period, mmcqd_rq_count[idx]);	//period %lld00 == period %lld x100
					pr_warn("[BLOCK_TAG] mmcqd:%d rw_time=%lld, discard_time=%lld, flush_time=%lld, rw_rq=%d, discard_rq=%d, flush_rq=%d\n", mmcqd[idx], mmcqd_cpu_t1[idx], mmcqd_cpu_t2[idx], mmcqd_cpu_t3[idx], mmcqd_rw_rq_count[idx], mmcqd_discard_rq_count[idx], mmcqd_flush_rq_count[idx]);
				}
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	unsigned int idx = find_mmcqd_index();
	time0 = time1 = sched_clock();
#endif
...
#if defined(FEATURE_STORAGE_PERF_INDEX)
		time2 = sched_clock();
		mmcqd_cpu_t2[idx] += time2 - time1;
		time1 = time2;
		mmcqd_discard_rq_count[idx]++;
#endif
...
#if defined(FEATURE_STORAGE_PERF_INDEX)
		time2 = sched_clock();
		mmcqd_cpu_t3[idx] += time2 - time1;
		time1 = time2;
		mmcqd_flush_rq_count[idx]++;
#endif
	} else {
		if (!req && host->areq) {
...
out:
	if ((!req && !(mq->flags & MMC_QUEUE_NEW_REQUEST)) ||
		...
		mmc_release_host(card->host);
#if defined(FEATURE_STORAGE_PERF_INDEX)
		mmcqd_thread_t1[idx] += sched_clock() - time0;
#endif
	return ret;
}
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if ((!sd_delay_cnt) && strncmp(mmc_hostname(card->host),"mmc0", 4)) {
		sd_delay_cnt++;
