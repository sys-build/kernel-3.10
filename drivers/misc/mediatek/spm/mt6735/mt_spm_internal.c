...
static u32 wake_reason_cnt[3][33] = {0};

void htc_get_wakeup_source_cnt(u32 index, char *buf)
{
	unsigned int len = 0;
	if(index == 32) {
		len += sprintf(buf + len, "index_%2d%8d\t%8d\t%8d\t TOTAL", index,
			wake_reason_cnt[0][index],
			wake_reason_cnt[1][index],
			wake_reason_cnt[2][index]
			);
	}
	else{
		len += sprintf(buf + len, "index_%2d%8d\t%8d\t%8d\t%s", index, 
			wake_reason_cnt[0][index],
			wake_reason_cnt[1][index],
			wake_reason_cnt[2][index],
			wakesrc_str[index]
				       ...
				       unsigned int src)
{
	...
	if(src > 2)
		src = 2;
	wake_reason_cnt[src][32]++;
	if (wakesta->r12 & WAKE_SRC_SPM_MERGE) {
		wake_reason_cnt[src][0]++;
		wake_reason_cnt_temp = wake_reason_cnt[src][0];
	...
	for (i = 1; i < 32; i++) {
		if (wakesta->r12 & (1U << i)) {
			wake_reason_cnt[src][i]++;
			wake_reason_cnt_temp = wake_reason_cnt[src][i];
	...
	spm_print(suspend, "wake up by%s, timer_out = %u(%ds), r13 = 0x%x, debug_flag = 0x%x, %d_(%d/%d)\n",
		  buf, wakesta->timer_out, (wakesta->timer_out >> 15), wakesta->r13, wakesta->debug_flag, src, wake_reason_cnt_temp, wake_reason_cnt[src][32]);
