...
static ssize_t store_LDO_STATUS(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if(test_item){
		ret1=regulator_enable(mreg->reg);
		if (ret1!=0)
		{
			PMICLOG("regulator_enable:%s enable fail",mreg->desc.name);
		}
	}
	else{
		ret1=regulator_disable(mreg->reg);
		if (ret1!=0)
		{
			PMICLOG("regulator_disable:%s disable fail",mreg->desc.name);
		}
	}
	PMICLOG("%s:%s %s\n",__func__,mreg->desc.name,(test_item)? "ON":"OFF");
...
void bat_l_int_handler(void)
{
	...
	// HTC++ in some condition vbat will be pulled low in an instant
	if(BMT_status.bat_vol <= 3600)
	{
		flag = 1;
	}
	// HTC--
...
{
	PMICLOG("[ldo_oc_int_handler]....\n");
	if(1 == pmic_get_register_value(PMIC_QI_VMCH_OCFB_EN))
	{
		PMICLOG("[VMCH OCFB] ....\n");
		msdc_sd_power_off();
	}
...
void PMIC_EINT_SETTING(void)
{
	...
	pmic_set_register_value(PMIC_RG_VMCH_OCFB_EN, 1); //Enable VMCH OCFB
...
ret = pmic_config_interface(0xF4A,0x0,0xF,0); //set cable out voltage to 4.2V
