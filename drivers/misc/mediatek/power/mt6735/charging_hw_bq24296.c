...
extern kal_uint16 htc_battery_get_AICL_status();
...
static kal_uint32 charging_hw_init(void *data)
{
	...
	if(htc_battery_get_AICL_status())
		bq24296_set_vindpm(0x4); //VIN DPM check 4.2V, for 1.5A charger, icey
	else bq24296_set_vindpm(0x8);
#else
	bq24296_set_vindpm(0x8); //VIN DPM check 4.52V, change from 4.68V for MT6763, icey
#endif
	bq24296_set_reg_rst(0x0);
	bq24296_set_wdt_rst(0x1); //Kick watchdog	
	bq24296_set_sys_min(0x5); //Minimum system voltage 3.5V	
	bq24296_set_iprechg(0x2); //Precharge current from 512 to 384mA, request from HW Kunwei, 2015/05/29
	...
	bq24296_set_en_term(0x0); //Disable termination
...
static void charging_powerpath_ctrl(void *data)
{
	kal_uint32 enable = *(kal_uint32*)(data);
	if(KAL_TRUE == enable)
		{
