...
#ifdef CONFIG_HTC_MODEM_EXCEPTION_INFO_EXPORT_C2K
#include <linux/rtc.h>
#endif
...
#ifdef CONFIG_HTC_MODEM_EXCEPTION_INFO_EXPORT_C2K
#define C2K_MODEM_EX_INFO_MAX_SIZE 512
struct proc_dir_entry *c2k_modem_ex_proc;
char c2k_modem_ex_info_msg[C2K_MODEM_EX_INFO_MAX_SIZE];

static int c2k_modem_proc_show(struct seq_file *m, void *v)
{
       char err_string[C2K_MODEM_EX_INFO_MAX_SIZE];
       LOGPRT(LOG_INFO, "%s~\n", __func__);

       memset(err_string, 0, sizeof(err_string));

       if(strlen(c2k_modem_ex_info_msg) > 0)
               snprintf(err_string, C2K_MODEM_EX_INFO_MAX_SIZE, "modem fatal:%s\n", c2k_modem_ex_info_msg);
       memset(c2k_modem_ex_info_msg, 0, sizeof(c2k_modem_ex_info_msg));

       seq_printf(m, "%s\n", err_string);
       return 0;
}

static int c2k_modem_ex_open(struct inode *inode, struct file *file)
{
       return single_open(file, c2k_modem_proc_show, NULL);
}

static const struct file_operations c2k_modem_ex_fops = {
       .open           = c2k_modem_ex_open,
       .read           = seq_read,
       .release        = single_release,
};

void c2k_modem_ex_proc_init(void)
{
       c2k_modem_ex_proc = proc_create("c2k_ex_info", 0664, NULL, &c2k_modem_ex_fops);
       if(c2k_modem_ex_proc == NULL) {
               LOGPRT(LOG_INFO, "fail to create proc entry\n");
       }
}

void c2k_modem_ex_proc_destory(void)
{
       if(c2k_modem_ex_proc != NULL) {
	       proc_remove(c2k_modem_ex_proc);
	       c2k_modem_ex_proc = NULL;
       }
}
#endif
...
static int modem_exception_handler(struct sdio_modem *modem)
{
	...
	if (excp_msg->ccci_head.data1 == C2K_MD_EX){
		...
		case MD_EX_TYPE_ASSERT:
			...
			memset(c2k_modem_ex_info_msg, 0, sizeof(c2k_modem_ex_info_msg));
			snprintf(c2k_modem_ex_info_msg, C2K_MODEM_EX_INFO_MAX_SIZE, "%d%02d%02d %02d:%02d:%02d.%u, exception type(%d):%s, %s, %d, %d, %d, %d\n",
