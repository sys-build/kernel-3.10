...
/*[SSD_RIL++]Print ccci tty data if device wakeup by CLDMA*/
...
static int cldma_rx_collect(struct md_cd_queue *queue, int budget, int blocking, int *result, unsigned long long *rxbytes)
{
    ...
    *rxbytes =0;
    ...
    while(1) { // not hardware own
		...
		/*[SSD_RIL++]print ccci CLDMA wakeup source port name*/
		...
		/*Print ccci tty data if device wakeup by CLDMA*/
