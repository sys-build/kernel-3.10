...
volatile int cur_eint_state = EINT_PIN_PLUG_OUT;//haley 20150512 modify for solving plug out headset result in device reboot
...
//Disable ACCDET
   ...
   /*haley 20150512 modify for solving plug out headset result in device reboot*/
#if 0 
#ifdef CONFIG_OF
...
#define PIN_ADC_CHANNEL 5
	...
	switch(accdet_status)
    {
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
			...
			mutex_lock(&accdet_eint_irq_sync_mutex);
			if(1 == eint_accdet_sync_flag) {
				ACCDET_DEBUG("Accdet key interrupt eint_accdet_sync_flag = %d\n", eint_accdet_sync_flag);
				...
				ACCDET_DEBUG("[Accdet] Headset  eint_accdet_sync_flag = %d\n", eint_accdet_sync_flag);
				irq_temp = pmic_pwrap_read(ACCDET_IRQ_STS);
				irq_temp = irq_temp & (~IRQ_CLR_BIT);
				pmic_pwrap_write(ACCDET_IRQ_STS, irq_temp);
            	IRQ_CLR_FLAG = TRUE;
			...
			mutex_unlock(&accdet_eint_irq_sync_mutex);
			msleep(30);
			ACCDET_DEBUG("[Accdet] Headset after sleep about 30ms the eint_accdet_sync_fla = %d\n", eint_accdet_sync_flag);
		    button_status = 1;
			if(button_status)
		    {	
			    mutex_lock(&accdet_eint_irq_sync_mutex);
