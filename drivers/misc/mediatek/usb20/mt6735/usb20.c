...
static bool usb_cable_status = 0;
static bool usb_disconnect_run_flag = 0;
...
static void mt_usb_disable(struct musb *musb)
{
	...
	if(0 == usb_cable_status){
		usb_disconnect_run_flag = 0;//don't care wheather run mt_usb_disconnect() or not
	}
	else{
		usb_disconnect_run_flag = 1;//need to run mt_usb_disconnect() if usb cable plug out
	}
...
{
	if (!mtk_musb || !mtk_musb->is_ready || mtk_musb->is_host || (!mtk_musb->power && !usb_disconnect_run_flag))
		return;
...
bool usb_cable_connected(void)
{
	...
	//if (upmu_is_chr_det()){
	    usb_cable_status = 1;
	    printk("usb_cable_connected usb_cable_status = %d\n",usb_cable_status);
		return true;
	}else{
	    usb_cable_status = 0;
	    printk("usb_cable_connected usb_cable_status = %d\n",usb_cable_status);
