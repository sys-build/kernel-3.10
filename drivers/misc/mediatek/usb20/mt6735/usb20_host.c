...
#if defined(CONFIG_USB_MTK_OTG)||defined(CONFIG_USB_HTC_ONLY_CARKIT_NO_OTG)
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	#ifdef CONFIG_USB_HTC_ONLY_CARKIT_NO_OTG
		//do nothing
...
#ifndef CONFIG_USB_HTC_GPIO_DET_CARKIT
...
#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT
	msleep(20);
#endif
...
#define GPIO_USB_ID_PIN         (GPIO61 | 0x80000000)

static bool is_htc_carkit=0;
static bool pre_htc_carkit_status=0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	id =  mt_get_gpio_in(GPIO_OTG_IDDIG_EINT_PIN);
...
static void set_otg_pin_to_iddig_mode(void)
{
	mt_set_gpio_mode(GPIO_OTG_IDDIG_EINT_PIN, GPIO_OTG_IDDIG_EINT_PIN_M_IDDIG);
	DBG(0,"set to GPIO_OTG_IDDIG_EINT_PIN_M_IDDIG done \n");
}
static void set_otg_pin_to_gpio_out_high(void)
{
	mt_set_gpio_mode(GPIO_OTG_IDDIG_EINT_PIN, GPIO_OTG_IDDIG_EINT_PIN_M_GPIO);
	...
	mt_set_gpio_out(GPIO_OTG_IDDIG_EINT_PIN, 1);
}
static bool read_gpio_usb_id_status(void)
{
	bool gpio_usb_id_status = 0;
	msleep(5);
	gpio_usb_id_status = mt_get_gpio_in(GPIO_USB_ID_PIN);
	DBG(0,"gpio_usb_id_status = %d\n",gpio_usb_id_status);

	return gpio_usb_id_status;
}
static bool htc_usb_gpio_det_carkit(void)
{
	set_otg_pin_to_gpio_out_high();
	msleep(5);
	if(1 == read_gpio_usb_id_status()){
		return true;
	}
	else{
		return false;
...
#endif//#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT

static void musb_id_pin_work(struct work_struct *data)
{
	u8 devctl = 0;
	unsigned long flags;
	static int acce_type = 0;
	int value = 0;

	if ( is_meta_mode() )
	{
		DBG(0, "do nothing due to meta mode\n");
		return;
	}
...
#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT
	DBG(0, "htc_carkit work start, is_htc_carkit=%d\n", is_htc_carkit);
	if(mtk_musb->is_host){ //otg_pin is low
		is_htc_carkit = htc_usb_gpio_det_carkit();
		DBG(0,"is_htc_carkit = %d\n",is_htc_carkit);
		if(true == is_htc_carkit){//car kit inserted
			mtk_musb->is_host = false;
			DBG(0,"[Cable]carkit inserted \n");
			switch_set_state(&dock_switch, DOCK_STATE_CAR);
			acce_type=DOCK_STATE_CAR;
			set_otg_pin_to_iddig_mode();
			switch_int_to_device(mtk_musb);
		#ifndef FPGA_PLATFORM
			DBG(0,"will unmask PMIC charger detection\n");
			pmic_chrdet_int_en(1);
		#endif
			pre_htc_carkit_status = 1;
			goto out;
		}
		else{//OTG cable inserted
			DBG(0,"OTG cable inserted \n");
			musb_platform_enable(mtk_musb);
			set_otg_pin_to_iddig_mode();
		}
	}
	else{ //otg_pin is high
		if(1 == pre_htc_carkit_status){
			...
			is_htc_carkit = 0;
			pre_htc_carkit_status = 0;
			DBG(0,"[Cable]carkit removed \n");
			goto out;
		}
	}
#endif//#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT
...
#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT
	DBG(0, "htc_carkit work end, is_htc_carkit=%d\n", is_htc_carkit);

	while((value = mfg_check_white_accessory(acce_type)) == -EAGAIN) {
...
#ifdef CONFIG_USB_HTC_GPIO_DET_CARKIT
	if (switch_dev_register(&dock_switch))
		printk("dock_switch switch_dev_register fail\n");
	else
		printk("dock_switch switch_dev register success\n");

	if (device_create_file(dock_switch.dev, &dev_attr_USB_ID_status))
		printk("[Cable] dev_attr_USB_ID_status failed\n");
	else
		printk("[Cable] dev_attr_USB_ID_status success\n");

#endif

}
#else//#if defined(CONFIG_USB_MTK_OTG)||defined(CONFIG_USB_HTC_ONLY_CARKIT_NO_OTG)
