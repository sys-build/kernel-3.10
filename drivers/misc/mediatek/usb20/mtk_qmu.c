...
static u32 Tx_Q_err_cnt[TXQ_NUM + 1];
#define TX_Q_ERR_THRESHOLD 5

static u32 Tx_Q_stop[TXQ_NUM + 1];
static u32 Rx_Q_stop[RXQ_NUM + 1];
...
void qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	if(Rx_Q_stop[ep_num]){
		QMU_WARN("RxQ %d already stop, directly return\n", ep_num);
		return;
	}
...
void qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	Tx_Q_err_cnt[ep_num] = 0;
	if(Tx_Q_stop[ep_num]){
		QMU_WARN("TxQ %d already stop, directly return\n", ep_num);
		return;
	}
	...
	list_for_each_entry(request, &musb_ep->req_list, list) {
		...
		if(request->request.dma != DMA_ADDR_INVALID)
		{
			if(request->tx)
			{
				...
				if(Tx_Q_err_cnt[ep_num] >= TX_Q_ERR_THRESHOLD) {

					/* stop retry, leave it to the class driver */
					if(request->request.length >= 0) {
						QMU_ERR("ERR, TX, try more than %d times, return -ECONNRESET to class driver\n", TX_Q_ERR_THRESHOLD);
						musb_g_giveback(musb_ep, &(request->request), -ECONNRESET);
					}else {
