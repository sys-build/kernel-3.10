                ...
                for(i = 0 ; i < prFwHead->u4NumOfEntries ; i++) {
                    ...
                    for(j = 0 ; j < prFwHead->arSection[i].u4Length ; j += CMD_PKT_SIZE_FOR_IMAGE) {
                        ...
                        if(wlanImageSectionDownload(prAdapter,
							...
							if (dumpid <= 9)
							{
								kalDnDump(prAdapter->prGlueInfo, (PUINT_8)pvFwImageMapFile + prFwHead->arSection[i].u4Offset + j, u4ImgSecSize);
								kalTxDump(prAdapter->prGlueInfo);
								kalSmDump(prAdapter->prGlueInfo);
								dumpid ++;
							}
#endif
