...
#define LOCAL_NIC_ALLOCATE_MEMORY(pucMem, u4Size, eMemType, pucComment) \
        { \
            ...
            DBGLOG(NIC, INFO, ("Virtual Address = %p for %s.\n", pucMem, pucComment)); \
        }
    ...
    )
{
    ...
    do {
        ...
        if(prAdapter->pucCoalescingBufCached == NULL) {
            DBGLOG(NIC, ERROR, ("Could not allocate %u bytes for coalescing buffer.\n", prAdapter->u4CoalescingBufCachedSize));
            break;
        }
        ...
        if(prAdapter->prSDIOCtrl == NULL) {
            DBGLOG(NIC, ERROR, ("Could not allocate %zu bytes for interrupt response.\n", sizeof(ENHANCE_MODE_DATA_STRUCT_T)));
            break;
        }
    ...
    )
{
    ...
    if (prAdapter->rAcpiState == ACPI_STATE_D3) {
        DBGLOG(NIC, WARN, ("Fail in set nicProcessIST! (Adapter not ready). ACPI=D%d, Radio=%d\n",
...
//        DBGLOG(INIT, TRACE, ("u4IntStatus: 0x%x\n", u4IntStatus));
        ...
        if (u4IntStatus & ~(WHIER_DEFAULT | WHIER_FW_OWN_BACK_INT_EN)) {
            DBGLOG(NIC, WARN, ("Un-handled HISR %#x, HISR = %#x (HIER:0x%x)\n",
    ...
    )
{
    ...
    DBGLOG(NIC, WARN, ("u4IntrBits: 0x%x \n", u4IntrBits));
    ...
    )
{
    ...
    DBGLOG(NIC, LOUD, ("nicUpdateBeaconIETemplate\n"));
    ...
    )
{
    ...
    DBGLOG(NIC, EVENT, ("sizeof(AC_QUE_PARMS_T): %zu \n", sizeof(AC_QUE_PARMS_T)));
    DBGLOG(NIC, EVENT, ("sizeof(CMD_UPDATE_WMM_PARMS): %zu \n", sizeof(CMD_UPDATE_WMM_PARMS_T)));
    DBGLOG(NIC, EVENT, ("sizeof(WIFI_CMD_T): %zu \n", sizeof(WIFI_CMD_T)));
