...
VOID scnEventNloDone(IN P_ADAPTER_T prAdapter, IN P_EVENT_NLO_DONE_T prNloDone)
{
	...
	if (prScanInfo->fgNloScanning == TRUE) {
        ...
        DBGLOG(SCN, INFO, ("scnEventNloDone reporting to uplayer; SeqNum = %d, prScanParam->ucSeqNum:%d Current State = %d\n", 
			prNloDone->ucSeqNum, prScanParam->ucSeqNum, prScanInfo->eCurrentState));
