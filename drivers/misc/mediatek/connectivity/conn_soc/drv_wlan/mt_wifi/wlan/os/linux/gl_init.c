...
extern BOOLEAN mtk_wcn_set_connsys_power_off_flag(BOOLEAN flag);
/*******************************************************************************
*                              C O N S T A N T S
********************************************************************************
*/
    ...
    )
{
    ...
    do {
...
#if CFG_SUPPORT_CFG_FILE
		{
			...
			if (!pucConfigBuf) {
				DBGLOG(INIT, ERROR, ("wlanProbe: No memory for config buf\n"));
	            i4Status = -ENOMEM;
	            break;
	        }
...
bailout:
            ...
            if (i4Status < 0) {
                ...
                /* dump HIF/DMA registers, if fgIsBusAccessFailed is FALSE, otherwise, dump HIF register may be hung */
				if (!fgIsBusAccessFailed)
...
//                    prGlueInfo->rHifInfo.DmaOps->DmaRegDump(&prGlueInfo->rHifInfo);
				...
				/* we don't care the return value of mtk_wcn_set_connsys_power_off_flag, because even this function returns
...
#if CFG_ENABLE_WIFI_DIRECT
{
    ...
    if ( i4Status == WLAN_STATUS_SUCCESS && (i4Status = kalMetInitProcfs(prGlueInfo)) < 0) {
...
//    prGlueInfo->u4Flag |= GLUE_FLAG_HALT;
	...
	DBGLOG(INIT, INFO, ("release tx_thread wakelock done\n"));
    KAL_WAKE_LOCK_DESTROY(prGlueInfo->prAdapter, &prGlueInfo->rAhbIsrWakeLock);
	DBGLOG(INIT, INFO, ("release ahbisr wakelock done\n"));
...
#endif
	DBGLOG(INIT, INFO, ("glUnregisterAmpc done\n"));

    //4 <3> Remove /proc filesystem.
#ifdef WLAN_INCLUDE_PROC
    procRemoveProcfs();
#endif /* WLAN_INCLUDE_PROC */
	DBGLOG(INIT, INFO, ("procRemoveProcfs() done\n"));
...
static int initWlan(void)
{
    ...
    aucDebugModule[DBG_INIT_IDX] |= DBG_CLASS_STATE |
