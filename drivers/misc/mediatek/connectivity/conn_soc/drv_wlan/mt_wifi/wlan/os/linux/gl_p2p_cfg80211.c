...
/* For ALPS02035004; we need to work around, thus we can't fix beacon interval to CFG_HOTSPOT_OPTIMIZATION_BEACON_INTERVAL */
    ...
    IN int len)
{
    ...
    if(data && len) {
		...
		DBGLOG(P2P, ERROR, ("mtk_p2p_cfg80211_testmode_p2p_sigma_cmd, data is NULL\n"));
		return -EINVAL;
	}
    ...
    IN int len)
{
    ...
    if(data && len) {
		...
		DBGLOG(P2P, ERROR, ("mtk_p2p_cfg80211_testmode_hotspot_block_cmd, data is NULL\n"));
		return -EINVAL;
	}
