    ...
    )
{
	...
	if (pParamConn->pucBssid) {
		if (!EQUAL_MAC_ADDR(aucZeroMacAddr, pParamConn->pucBssid) &&
			...
			DBGLOG(OID, TRACE, ("wrong bssid "MACSTR"to connect\n", MAC2STR(pParamConn->pucBssid)));
		}
	} else
    ...
    DBGLOG(OID, TRACE, ("ssid %s, bssid "MACSTR", conn policy %d, disc reason %d\n",
