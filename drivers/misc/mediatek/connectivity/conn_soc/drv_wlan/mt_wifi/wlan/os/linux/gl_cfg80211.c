...
//            prGlueInfo->ulFlag |= GLUE_FLAG_FRAME_FILTER_AIS;
            ...
            if (in_interrupt())
                DBGLOG(INIT, LOUD, ("It is in interrupt level\n"));
        }
    ...
    )
{
    ...
    DBGLOG(SCN, INFO, ("mtk_cfg80211_sched_scan_start  \n"));    
    ...
    )
{
	...
	DBGLOG(SCN, INFO, ("prGlueInfo->prSchedScanRequest == NULL\n"));
        return -EBUSY;
    }
    ...
    if (rStatus != WLAN_STATUS_SUCCESS) {
        DBGLOG(REQ, WARN, ("scheduled scan error, rStatus: %d\n", rStatus));
			return -EINVAL;
    }
    ...
    DBGLOG(SCN, INFO, ("sched scan stopped\n"));
    return 0;
}
    ...
    IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if(data && len) {
		...
		DBGLOG(REQ, ERROR, ("mtk_cfg80211_testmode_get_sta_statistics, data is NULL\n"));
		return -EINVAL;
	}
    ...
    IN int len)
{
    ...
    if(data && len) {
		...
		DBGLOG(REQ, ERROR, ("mtk_cfg80211_testmode_hs20_cmd, data is NULL\n"));
		return -EINVAL;
	}
