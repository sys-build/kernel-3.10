...
UINT_32 u4TempBuf[25000];
#define Sm_REG_READL(_addr)       (*((volatile UINT32 *)(_addr)))
extern int dumpid;
    ...
    )
{
    ...
    int retLen = -1, i;
	UINT_64 addr1;
	UINT_8 filename[20] = "/data/htclog/csmb1";

    mm_segment_t old_fs = get_fs();
    set_fs(KERNEL_DS);

	filename[17] = 0x30 + dumpid;
//    fd = filp_open("/data/htclog/csm1", O_WRONLY|O_CREAT, 0644);
	fd = filp_open(filename, O_WRONLY|O_CREAT, 0644);

    if(IS_ERR(fd)) {
        DBGLOG(INIT, INFO, ("[csm_write] : failed to open!!\n"));
        return;
    }

    do{
        if ((fd->f_op == NULL) || (fd->f_op->write == NULL)) {
            DBGLOG(INIT, INFO, ("[csm_write] : file can not be write!!\n"));
            break;
        } /* End of if */
        ...
        if (fd->f_pos != 0) {
            if (fd->f_op->llseek) {
                if(fd->f_op->llseek(fd, 0, 0) != 0) {
                    DBGLOG(INIT, INFO, ("[csm_write] : failed to seek!!\n"));
                    break;
                }
            }
            else {
    ...
    )
{
    ...
    int retLen = -1, i, j=0;
	UINT_64 addr1 = 0;
	UINT_8 filename[20] = "/data/htclog/csma1";

	/* read buf */
	kalMemZero(u4TempBuf, sizeof(u4TempBuf));
#if 1
	//addr1 = ioremap(0x18090000, 0x17000);
//	addr1 = CONN_MCU_CONFIG_BASE + 0x20000;
	addr1 = ioremap(0x18090000, 0x17000);
	//addr1 = phys_to_virt(0x18090000);
	printk("<samp> addr1 = 0x%llx\n", addr1);
	for(i=0; i<0x17000; i+=4)
		u4TempBuf[j++] = Sm_REG_READL(addr1+i);
	//iounmap(addr1);
#endif
    mm_segment_t old_fs = get_fs();
    set_fs(KERNEL_DS);
	...
	filename[17] = 0x30 + dumpid;
//    fd = filp_open("/data/htclog/csm", O_WRONLY|O_CREAT, 0644);

	if (fgDumpSpec == FALSE)
		fd = filp_open(filename, O_WRONLY|O_CREAT, 0644);
	else
		fd = filp_open("/data/htclog/csmdmafail", O_WRONLY|O_CREAT, 0644);
	...
	iounmap(addr1);
#if 0
	/* read /write SRAM test */
	printk("<mtk> SRAM test start...\n");
//	addr1 = CONN_MCU_CONFIG_BASE + 0x20000; /* 0xf8090000 */
	addr1 = ioremap(0x18090000, 0x17000);
	for(i=0; i<0x16000; i+=4)
	{
		addr1 += 4;
		Sm_REG_READL(addr1) = 0x5a5a5a5a;
		u4TempBuf[0] = Sm_REG_READL(addr1);
		if (u4TempBuf[0] != 0x5a5a5a5a)
			printk("<mtk> SRAM test fail in 0x%x addr 0x%llx val 0x%x\n", i, addr1, u4TempBuf[0]);
	}
	iounmap(addr1);
#endif
...
{
	extern UINT_8 ucBackTxBuf[];
	extern UINT_32 ucBackLen;
    struct file *fd;
    int retLen = -1, i;
	UINT_64 addr1;
	UINT_8 filename[20] = "/data/htclog/csmd1";

    mm_segment_t old_fs = get_fs();
    set_fs(KERNEL_DS);

	filename[17] = 0x30 + dumpid;
//    fd = filp_open("/data/htclog/csm2", O_WRONLY|O_CREAT, 0644);
	fd = filp_open(filename, O_WRONLY|O_CREAT, 0644);
