	...
	struct file	*pFile)
{
	...
	LOG_WRN("- E. UserCount: %d. O", IspInfo.UserCount);
	/*	*/
	spin_lock(&(IspInfo.SpinLockIspRef));
	/*	*/
	...
	if (IspInfo.UserCount >	0) {
		IspInfo.UserCount++;
		LOG_WRN("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d),	users exist",
...
EXIT:
	...
	LOG_WRN("- X. Ret: %d. UserCount: %d. O", Ret, IspInfo.UserCount);
	return Ret;
	...
	struct file	*pFile)
{
	ISP_USER_INFO_STRUCT *pUserInfo;
	MUINT32	Reg;
	LOG_WRN("- E. UserCount: %d. R", IspInfo.UserCount);
	/*	*/
...
EXIT:
	...
	LOG_WRN("- X. UserCount: %d. R", IspInfo.UserCount);
	return 0;
}
