...
//#ifdef MUX_LOG
    //clk_info("[%s]: mux->name=%s\n", __func__, mux->name);
    //printk("[%s]: mux->name=%s, CLK_CFG_4=0x%x, AUDIO_TOP_CON0=0x%x\n", __func__, mux->name, clk_readl(CLK_CFG_4), clk_readl(AUDIO_TOP_CON0));
    clk_dbg("[%s]: mux->name=%s\n", __func__, mux->name);
//#endif
...
{
    printk("[%s]: 0x%x, 0x%x\n", __func__, clk_readl(CLK_CFG_4),clk_readl(AUDIO_TOP_CON0));
    clk_clrl(clk->grp->sta_addr, clk->mask);
    printk("[%s+1]: 0x%x, 0x%x\n", __func__, clk_readl(CLK_CFG_4),clk_readl(AUDIO_TOP_CON0));
    return 0;
}
