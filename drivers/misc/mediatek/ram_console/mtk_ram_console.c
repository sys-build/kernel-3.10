...
void sram_log_save(const char *msg, int count)
{
	...
	//If device shutdown and bootup in a short time. The value of buffer->start might be wrong in low temp environment(ex: -20 degrees Celcius)
	//1. buffer->start = 0x1002F282 or 0x1008840f => 16MB
	//2. ram_console_buffer size is only 1MB
...
static int ram_console_lastk_show(struct ram_console_buffer *buffer, struct seq_file *m, void *v)
{
   ...
   if(bl_last_log_buf == NULL || bl_last_log_buf_size == 0) {
		   ...
		   seq_puts(m, "ram_console: failed to get bl last log buf\n");
   }
   else
	{
		seq_puts(m, ">>>>>last bootloader log:<<<<<\n") ;
   ...
   if(bl_cur_log_buf == NULL || bl_cur_log_buf_size == 0) {
		   ...
		   seq_puts(m, "ram_console: failed to get bl cur log buf\n");
   }
   else
   {
		seq_puts(m, ">>>>>current botloader log:<<<<<<\n") ;
...
#if defined(CONFIG_HTC_DEBUG_BOOTLOADER_LOG)
	/*don't release bootloader log in mt6753 projects,
	 *in which bl_last_log_buf and bl_cur_log_buf don't be copyied to ram_console_old_log*/
//    bldr_log_release();
#endif
	return 0;
}
