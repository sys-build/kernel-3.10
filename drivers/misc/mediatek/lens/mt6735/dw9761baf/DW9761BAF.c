...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("DW9761BAF", 0x1b)};// add by zhaofei - 2014-03-18-16-32
...
static int s4DW9761BAF_WriteReg(u16 a_u2Data)
{
    ...
    DW9761BAFDB("[DW9761BAF]  write %d \n",  a_u2Data);
		...
		//mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN,GPIO_DIR_OUT);
		//mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,1);
    //g_pstDW9761BAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstDW9761BAF_I2Cclient, puSendCmd, 2);
    //DW9761BAFDB("[DW9761BAF]i4RetValue[%d]\n", i4RetValue);
    puSendCmd[0] = 0x04;
...
/*        if(ret == 0)*/
/*        {*/
/*            DW9761BAFDB("[DW9761BAF] Init Pos %6d \n", InitPos);*/
...
#if 0  //add longcheer tianyaping 20140508
...
static int DW9761BAF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	//puSendCmd3[2] = {(char)(0x05),(char)(0x65)};
	...
	#if 1 // module OLQBA21 damping para tommy.wan 20150704
   	puSendCmd2[0] = 0x06;
   	puSendCmd2[1] = 0x61;
   	puSendCmd3[0] = 0x07;
   	puSendCmd3[1] = 0x36;
	#else // other module damping para tommy.wan 20150704
   	...
   	else if (VcmID == 0x02)   // for VCM MTS
   	{
   		puSendCmd2[0] = 0x06;
	   	puSendCmd2[1] = 0x61;
	   	puSendCmd3[0] = 0x07;
   	...
   	else if (VcmID == 0x03)   // for VCM SIKAO
   	{
   		puSendCmd2[0] = 0x06;
	   	puSendCmd2[1] = 0x60;
	   	puSendCmd3[0] = 0x07;
    ...
    #if 0  //add longcheer tianyaping 20140508
    s4DW9761BAF_WriteReg2(0x02,0x02); //Ring control mode
    s4DW9761BAF_WriteReg2(0x06,0x60); //SAC3 mode,PRESC(Tvib * 2)
    s4DW9761BAF_WriteReg2(0x07,0x03); //Tvib=13.2ms(6.6ms * 2)
    #endif
...
static int DW9761BAF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    g_pstDW9761BAF_I2Cclient->addr = DW9761BAF_VCM_WRITE_ID;// add by zhaofei - 2014-03-18-16-32
