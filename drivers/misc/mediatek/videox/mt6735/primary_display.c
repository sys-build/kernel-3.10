...
static unsigned char shrink_pwm(unsigned int val)
{
	...
	DISPCHECK("brightness orig=%d, transformed=%d\n", val, shrink_br);
...
int primary_display_setlcm_cmd(unsigned int *lcm_cmd, unsigned char* cmd_num,unsigned char *lcm_value)
{
    int ret=0;
    DISPFUNC();
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				disp_lcm_set_cmd(pgc->plcm, (void*) cmdq_handle_cmd,lcm_cmd,cmd_num,lcm_value);
