...
static int module_init_emmc = 0;
static int module_init_sd = 0;
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
	...
	if (host->id ==0) {//Need modify by platform
		pr_err("R[D4]=0x%x	R[F0]=0x%x	R[F8]=0x%x R[FC]=0x%x  R[110]=0x%x	R[114]=0x%x  R[118]=0x%x\n",
				sdr_read32(base + 0xD4), sdr_read32(base + 0xF0),
				sdr_read32(base + 0xF8), sdr_read32(base + 0xFC),
				sdr_read32(base + 0x110), sdr_read32(base + 0x114),
				sdr_read32(base + 0x118));
	} if (host->id ==1) {
		pr_err("R[D4]=0x%x	R[F0]=0x%x	R[F4]=0x%x R[F8]=0x%x  R[FC]=0x%x  R[110]=0x%x  R[114]=0x%x  R[118]=0x%x\n",
				sdr_read32(base + 0xD4), sdr_read32(base + 0xF0),
				sdr_read32(base + 0xF4), sdr_read32(base + 0xF8), 
				sdr_read32(base + 0xFC), sdr_read32(base + 0x110),
...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
    ...
    switch (host->id) {
        case 0:
			sdr_set_field(MSDC0_GPIO_DRV0_G5_ADDR,MSDC0_DRV_CMD_MASK, msdc0_hw.cmd_drv);
			sdr_set_field(MSDC0_GPIO_DRV0_G5_ADDR,MSDC0_DRV_DSL_MASK, msdc0_hw.cmd_drv);
			sdr_set_field(MSDC0_GPIO_DRV0_G5_ADDR,MSDC0_DRV_CLK_MASK, msdc0_hw.clk_drv);
			sdr_set_field(MSDC0_GPIO_DRV0_G5_ADDR,MSDC0_DRV_DAT_MASK, msdc0_hw.dat_drv);
			sdr_set_field(MSDC0_GPIO_DRV0_G5_ADDR,MSDC0_DRV_RSTB_MASK, msdc0_hw.cmd_drv);
        ...
        case 1:
			...
			sdr_set_field(MSDC1_GPIO_DRV0_G4_ADDR,MSDC1_DRV_CMD_MASK, msdc1_hw.cmd_drv);
			sdr_set_field(MSDC1_GPIO_DRV0_G4_ADDR,MSDC1_DRV_CLK_MASK, msdc1_hw.clk_drv);
			sdr_set_field(MSDC1_GPIO_DRV0_G4_ADDR,MSDC1_DRV_DAT_MASK, msdc1_hw.dat_drv);
