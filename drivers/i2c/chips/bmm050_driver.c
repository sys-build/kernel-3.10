...
static struct bmm_client_data *g_client_data;
		...
		struct bmm_client_data *client_data)
{
	...
	client_data->bst_pd->place = g_client_data->bst_pd->place;
...
#ifdef CONFIG_BMM_USE_PLATFORM_DATA
	if (NULL != g_client_data->bst_pd)
		place = g_client_data->bst_pd->place;
#endif
	return sprintf(buf, "%d\n", place);
}
...
static int bmm_parse_dt(struct device *dev, struct bosch_sensor_specific *bst_pd)
{
	...
	if (bst_pd == NULL) {
		PERR("%s: bst_pd is NULL\n", __func__);
		return -EINVAL;
	}

#ifdef CONFIG_BMM_USE_PLATFORM_DATA
	prop = of_find_property(dt, "compass_bmc156,place", NULL);
	if (prop) {
		of_property_read_u32(dt, "compass_bmc156,place", &buf);
		bst_pd->place = buf;
		PINFO("%s: place = %d", __func__, bst_pd->place);
	} else
		PINFO("%s: compass_bmc156,place not found", __func__);
#endif
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_BMM_USE_PLATFORM_DATA
	client_data->bst_pd = kzalloc(sizeof(struct bosch_sensor_specific), GFP_KERNEL);
	if (NULL == client_data->bst_pd) {
		PERR( "failed to allocate memory for client_data->bst_pd\n");
		goto bst_pd_kmalloc_fail;
	}
#endif
	...
	if (client->dev.of_node) {
		...
		if (err < 0)
			goto exit_err_parse_dt;
	} else {
		PINFO("Has No Device Tree.\n");
	}

	PINFO("Chip place: %d", client_data->bst_pd->place);
