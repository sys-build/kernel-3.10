...
static irqreturn_t pn544_dev_irq_handler(int irq, void *dev_id)
{
	...
	if (mt_get_gpio_in(pn544_dev->irq_gpio) == 0) {
		I("%s: irq_workaround\n", __func__);
		return IRQ_HANDLED;
	}
		...
		size_t count, loff_t *offset)
{
	...
	val = mt_get_gpio_in(pni->irq_gpio);
	...
	if (!mt_get_gpio_in(pni->irq_gpio)) {
		...
		ret = wait_event_interruptible(pni->read_wq, mt_get_gpio_in(pni->irq_gpio));
				...
				const char *buf, size_t count)
{
	...
	I("%s: irq = %d,  ven_gpio = %d,  firm_gpio = %d +\n", __func__, \
		mt_get_gpio_in(pni->irq_gpio), pn544_isEn(), \
		mt_get_gpio_out(pni->firm_gpio));
	I("%s: store value = %d\n", __func__, code);
	...
	I("%s: irq = %d,  ven_gpio = %d,  firm_gpio = %d -\n", __func__, \
		mt_get_gpio_in(pni->irq_gpio), pn544_isEn(), mt_get_gpio_out(pni->firm_gpio));
	return count;
}
...
//for DMA I2C adderss --
	...
	D("%s: Set VEN pin  %d chk value %d\n", __func__, pni->ven_gpio, mt_get_gpio_out(pni->ven_gpio));
	...
	D("%s: Set IRQ pin	%d chk value %d\n", __func__, pni->irq_gpio, mt_get_gpio_in(pni->irq_gpio));
	...
	D("%s: Set FWDL pin	%d chk value %d\n", __func__, pni->firm_gpio, mt_get_gpio_out(pni->firm_gpio));
	...
	if (is_alive == 0) {
		...
		if(pni->boot_mode == NFC_BOOT_MODE_OFF_MODE_CHARGING ) {
			...
			pn544_htc_pvdd_off();
			msleep(5);
			mt_set_gpio_out(pni->ven_gpio, GPIO_OUT_ONE);
#endif
		}
	}
...
static int pn544_suspend(struct i2c_client *client, pm_message_t state)
{
	struct pn544_dev *pni = pn_info;
        ...
        I("%s: irq = %d, ven_gpio = %d, isEn = %d, isReadBlock =%d\n", __func__, \
                mt_get_gpio_in(pni->irq_gpio), mt_get_gpio_out(pni->ven_gpio), pn544_isEn(), pni->isReadBlock);
