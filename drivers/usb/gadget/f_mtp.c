				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	unsigned long	flags;
	int	id;/*++ 2015/04/15 USB Team, PCN00031 ++*/
	...
	if (ctrl->bRequestType ==
		...
		DBG(cdev, "class request: %d index: %d value: %d length: %d\n",
			ctrl->bRequest, w_index, w_value, w_length);
		id = mtp_interface_desc.bInterfaceNumber;/*++ 2015/04/15 USB Team, PCN00031 ++*/

		if (ctrl->bRequest == MTP_REQ_CANCEL
#ifndef CONFIG_MTK_TC1_FEATURE
                                 && (w_index == 0 || w_index == id)/*++ 2015/04/15 USB Team, PCN00031 ++*/
#endif
				&& w_value == 0) {
...
void mtp_mac_descrp_set()
{
	struct mtp_dev *dev = _mtp_dev;

	printk(KERN_INFO "mtp_mac_descrp_set\n");
...
static int mtp_bind_config(struct usb_configuration *c, bool ptp_config)
{
	...
	printk(KERN_INFO "%s: ptp_config=%d, fsg_mode=%d, mac_mtp_mode=%d \n", __func__, ptp_config, fsg_mode, mac_mtp_mode);
	if (ptp_config || fsg_mode || !mac_mtp_mode) {/*++ 2015/04/23, USB Team, PCN00033 ++*/
