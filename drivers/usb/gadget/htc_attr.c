...
{
	// 2015/03/02, USB Team, PCN00037, Add "factory" to supported list
	...
	const char *buf, size_t size)
{
	...
	if (android_dev->connected)
		schedule_delayed_work(&cdev->request_reset,REQUEST_RESET_DELAYED);
	cdev->do_serial_number_change = true; /*2015/03/03, USB Team, PCN00037*/
			    ...
			    const char *buff, size_t size)
{
	int value;
	sscanf(buff, "%d", &value);
	/* only allow usb_disable to be 0 or 1*/
	if (value==1 || value==0) {
