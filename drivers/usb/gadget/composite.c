...
static int usb_autobot_mode(void);/*++ 2015/04/23, USB Team, PCN00033 ++*/
static void mtp_update_mode(int _mac_mtp_mode);/*++ 2015/04/23, USB Team, PCN00033 ++*/
static void fsg_update_mode(int _linux_fsg_mode);/*++ 2015/04/23, USB Team, PCN00033 ++*/
void usb_android_force_reset(struct usb_composite_dev *cpdev);
extern void mtp_mac_descrp_set();
...
/*++ 2015/04/23, USB Team, PCN00033 ++*/
		...
		if (os_type == OS_LINUX && is_mtp_enable) {
			...
			mtp_update_mode(0);
			fsg_mode = 1;
		} else if(os_type == OS_MAC && is_mtp_enable) {
			fsg_update_mode(0);
			mtp_update_mode(1);
			fsg_mode = 0;
			mtp_mac_descrp_set();
/*++ 2015/03/03, USB Team, PCN00037 ++*/
...
/*++ 2015/04/10, USB Team, PCN00011 ++*/
			if (w_length == 4) {
				pr_info("%s: OS_MAC\n", __func__);
				os_type = OS_MAC;
				mtp_update_mode(1);/*++ 2015/04/23, USB Team, PCN00033 ++*/
				mtp_mac_descrp_set();
			} else if (w_length == 255) {
				pr_info("%s: OS_WINDOWS\n", __func__);
				os_type = OS_WINDOWS;
			} else if (w_length == 9 && os_type != OS_WINDOWS && os_type !=OS_MAC) {
