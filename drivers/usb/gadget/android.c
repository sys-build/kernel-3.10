...
#include<linux/htc_devices_dtb.h>/*++ 2015/04/23, USB Team, PCN00034 ++*/
...
void usb_android_force_reset(struct usb_composite_dev *cpdev)
{
	struct android_dev *dev = _android_dev;
	printk("usb_android_force_reset start\n");
	mutex_lock(&dev->mutex);
	/* force reenumeration */
	if (cpdev && cpdev->gadget && cpdev->gadget->speed != USB_SPEED_UNKNOWN) {
		android_disable(dev);
	...
	printk("usb_android_force_reset end\n");
}

/*++ 2015/04/15 USB Team, PCN00027 ++*/
...
/*++ 2015/04/15, USB Team, PCN00030 ++*/
			// index4 is for specific usage, do not clear port_index.
			if (i != 4)
...
static char serial_transports[108];	/*enabled FSERIAL ports - "tty[,sdio]"*/
			       ...
			       const char *buff, size_t size)
{
	...
	is_mtp_enable = 0;/*++ 2015/04/23, USB Team, PCN00033 ++*/
...
static int spcf_flag = 0;// 0: common sku; >=1: specific sku; 2: factory2/meta/ftm
static int usb_sim_trigger =1;// 1: enable USB function switch; 0: block USB function switch
...
/*++ 2015/02/09, USB Team, PCN00034 ++*/
	/* only allow specific sku to block usb */
			    ...
			    const char *buff, size_t size)
{
	int value;
	sscanf(buff, "%d", &value);
	/* only allow usb_sim_trigger to be 0 or 1*/
	if (value==1 || value==0) {
	/* Once BSP provide an API, we can store the value to cmdline, and we can read it in init() */
...
static DEVICE_ATTR(usb_sim_trigger, S_IRUGO | S_IWUSR, sim_trigger_show, sim_trigger_store);/*++ 2015/02/09, USB Team, PCN00034 ++*/
...
static struct device_attribute *android_usb_attributes[] = {
	...
	&dev_attr_enable,
	&dev_attr_state,
	&dev_attr_usb_sim_trigger,/*++ 2015/02/09, USB Team, PCN00034 ++*/
	NULL
};
...
/*++ 2015/04/23, USB Team, PCN00034 ++*/
	...
	if (!strcmp(htc_get_bootmode(),"factory2") || !strcmp(htc_get_bootmode(),"meta") || !strcmp(htc_get_bootmode(),"ftm") || !strcmp(htc_get_bootmode(),"RUU")) {
		...
		/* Once BSP provide an API, we can determine the initial value of usb_sim_trigger by that API*/
			spcf_flag = 1;
			usb_sim_trigger =0;
