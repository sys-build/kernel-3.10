...
static int g_ovp_det_gpio;
static int g_ovp_irq;
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	if ((bat_data->BAT_batt_temp > 550) && (BMT_status.keep_charger_on || BMT_status.flag_pa_fake_batt_temp))
		bat_data->BAT_batt_temp = 550;
	...
	if((BMT_status.temperature < -10) && (BMT_status.bat_vol < 3800)
		 && g_display_on && !BMT_status.keep_charger_on) {
		 if(BMT_status.temperature < -16)
		 	bat_data->BAT_CAPACITY = 0;
		 else if(fgauge_get_battery_id() == 2)
		    bat_data->BAT_CAPACITY = 0;
	}
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
...
#else
	kal_uint32 cust_sync_time = SYNC_TO_REAL_TRACKING_TIME;
	static kal_uint32 timer_counter = (SYNC_TO_REAL_TRACKING_TIME / BAT_TASK_PERIOD);
#endif
	...
	if (BMT_status.bat_full == KAL_TRUE)	/* charging full first, UI tracking to 100% */
	{
		...
		battery_log(BAT_LOG_CRTI, "[100percent], UI_SOC(%d), reset(%d)\n",
	...
	} else if(htc_battery_100percent_early()){  /* HTC added this function  ++ */
			...
			resetBatteryMeter = KAL_TRUE;   /* HTC added this function  -- */
	} else {
...
static void ac_update(struct ac_data *ac_data)
{
	...
	if((!mt_get_gpio_in_base(GPIO125) || BMT_status.charger_exist == KAL_TRUE)
		&& battery_meter_get_charger_voltage() > 4000){
		#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		if ((BMT_status.charger_type == NONSTANDARD_CHARGER) ||
...
int htc_batt_update_limited_charge(INT32 temperature)
{
	...
	if (g_display_on == KAL_FALSE) {
		...
		if ((!(g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_THRML)) && temperature > 39
			...
			temp_soc = BMT_status.UI_SOC;
			g_chg_limit_reason |= HTC_BATT_CHG_LIMIT_BIT_THRML;
		} else if ((g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_THRML) &&
			((temperature <= 37) ||
			((temp_soc - BMT_status.UI_SOC) >= 20)||
			(BMT_status.UI_SOC < 30))){
			g_chg_limit_reason &= ~HTC_BATT_CHG_LIMIT_BIT_THRML;
		}else
			{
...
kal_uint16 htc_charging_avg_voltage_check()
{
...
kal_uint16 htc_battery_get_AICL_status()
{
...
#if defined(MTK_TEMPERATURE_RECHARGE_SUPPORT)
	/* The temperature is the average value, it causes can not stop charge on time. */
	/* We uses temperature_now to check temperature stop charging behavior */
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
			    || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
			    if(mt_get_gpio_in_base(GPIO125))
				{
						battery_log(BAT_LOG_CRTI,
...
static irqreturn_t ovp_operation_thread(int irq, void *cookie)
{
#if defined(V36BML_BATT)
	int mGPIO125 = mt_get_gpio_in_base(GPIO125);
	int mGPIO66 = mt_get_gpio_in_base(GPIO66);
	if(!mGPIO66){
		irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_HIGH);
		if(mGPIO125){
				is_ovp = 1;
				BMT_status.bat_charging_state = CHR_ERROR;
				BMT_status.charger_protect_status = charger_OVER_VOL;
		...
		irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_LOW);
		is_ovp = 0;
		BMT_status.charger_protect_status = 0;
		BMT_status.bat_charging_state = CHR_PRE;
	}
	battery_log(BAT_LOG_CRTI,"[%s]mGPIO125=%d,mGPIO66=%d\n",__FUNCTION__,mGPIO125,mGPIO66);
#else
	int mGPIO125 = mt_get_gpio_in_base(GPIO125);
	kal_int32 mChargerVol = battery_meter_get_charger_voltage();
	if(!mGPIO125)
		{
			irq_set_irq_type(g_ovp_irq, IRQF_TRIGGER_HIGH);
			if(mChargerVol < 4300 || mChargerVol > 6200)
				{
					is_ovp = 1;
					BMT_status.bat_charging_state = CHR_ERROR;
					BMT_status.charger_protect_status = charger_OVER_VOL;
	...
	battery_log(BAT_LOG_CRTI,"[%s]mGPIO125=%d,mChargerVol=%d\n",__FUNCTION__,mGPIO125,mChargerVol);
#endif
	wake_up_bat();
	return IRQ_HANDLED;
}
...
static int htc_ovp_det_init(struct platform_device *dev)
{
	...
	#if defined(A51BML_BATT) || defined(A51CML_BATT) || defined(V36BML_BATT)
		rc = request_threaded_irq(g_ovp_irq, NULL,
		     ovp_operation_thread,
		      EINTF_TRIGGER_LOW|IRQF_ONESHOT ,"OVP_DET-eint" , NULL);
	#else
		rc = request_irq(g_ovp_irq, ovp_det_handler, EINTF_TRIGGER_FALLING|EINTF_TRIGGER_RISING, "OVP_DET-eint", NULL);
	#endif
	return 0;
}
static int battery_probe(struct platform_device *dev)
{
	...
	//htc_battery_para_init(); // init it ASAP during battery driver probe, move from pmic.c
	/* Integrate with NVRAM */
  ...
  #if defined(A51BML_BATT) || defined(A51CML_BATT)
  #else
	battery_charging_control(CHARGING_CMD_HTC_AICL_INIT, NULL); // Pull GPIO 121 only
  #endif
#endif
	/* HTC feature end */
...
static void htc_batt_late_resume(struct early_suspend *h)
{
	...
	htc_battery_limited_power();
#endif
	/* To notify shutdown in time when display on */
	if(BMT_status.temperature < -10 && BMT_status.bat_vol < 3800) {
