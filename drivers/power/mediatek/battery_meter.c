...
kal_bool  gFG_ever_full_charged = KAL_FALSE;
kal_bool  gFG_rtc_used = KAL_FALSE;
...
kal_int32 gFG_capacity_by_hwocv_int = 0;
...
int force_get_tbat(kal_bool update)
{
	...
	if ((bat_temperature_val > 55) && (BMT_status.test_power_monitor || BMT_status.keep_charger_on))
		return 55;
...
void fgauge_construct_battery_profile_init(void)
{
	...
	saddles = fgauge_get_saddles();
	temp_profile_p = (BATTERY_PROFILE_STRUC_P) kmalloc(ZCV_TABLE_SIZE * sizeof(*temp_profile_p), GFP_KERNEL);
...
void fgauge_construct_r_table_profile(kal_int32 temperature, R_PROFILE_STRUC_P temp_profile_p)
{
	...
	for (i = 0; i< saddles-1; i++) {
		delta_v = (temp_profile_p+i)->voltage - (temp_profile_p+i+1)->voltage;
		delta_r = (temp_profile_p+i+1)->resistance - (temp_profile_p+i)->resistance;
		if (delta_r < 0){
			(temp_profile_p+i+1)->resistance = (temp_profile_p+i)->resistance;
			continue;
		}
		if (2*delta_r > delta_v)
			(temp_profile_p+i+1)->resistance = (temp_profile_p+i)->resistance + 2*delta_r;
	}
...
void oam_init(void)
{
	...
	kal_uint32 batfet_status = 1;
#endif

	/*stop charging for vbat measurement */
#ifdef MTK_BQ24296_SUPPORT
	battery_charging_control(CHARGING_CMD_SET_BATFET_STATUS, &batfet_status);
#else
...
/* 5. Logging */
	bm_print(BM_LOG_CRTI,
		 "[FGADC] Curr: %c%d, CC: %d, TCC:%c%d, (gFG_voltage): %d, SOC_by_v: %d, SOC_by_c: %d,%d,%d, Caging: %d,%d,%d,%d,%d,%d,%d, D0: %d, D1: %d,%d,%d\r\n",
		 gFG_Is_Charging>0?'+':'-' , gFG_current, gFG_columb, gFG_ever_full_charged==KAL_TRUE?'f':' ',(gFG_columb+gFG_columb_save),
		 gFG_voltage, gFG_capacity_by_v,gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
...
void fgauge_algo_run_init(void)
{
	...
	for (i=0;i<50;i++){
		ret=battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT_SIGN, &gFG_Is_Charging);
		if (gFG_Is_Charging <= 0){
			bm_print(BM_LOG_CRTI,"Charger disable.");
			break;
		}
		msleep(10);
	}
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if (vbat_val > VBAT_NORMAL_WAKEUP)	/* 3.6v */
	{
		...
		if (g_chg_limit_reason & HTC_BATT_CHG_LIMIT_BIT_TALK)
			...
			g_spm_timer = NORMAL_WAKEUP_PERIOD; /* 1 hour */
	} else if (vbat_val > VBAT_LOW_POWER_WAKEUP) {	/* 3.5v */
		g_spm_timer = LOW_POWER_WAKEUP_PERIOD;	/* 5 min */
	} else {
		g_spm_timer = CLOSE_POWEROFF_WAKEUP_PERIOD;	/* 0.5 min */
