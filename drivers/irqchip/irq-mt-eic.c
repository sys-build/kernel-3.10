...
static unsigned int c2k = 0;
static unsigned int pmic = 0;
void htc_get_wakeup_eint_cnt(u32 index, char *buf)
{
	unsigned int len = 0;
	if(index == 212) {
		len += sprintf(buf + len, "by c2k  eint_%03d = %3d", index, c2k);
	}
	else if(index == 206){
		len += sprintf(buf + len, "by pmic eint_%03d = %3d", index, pmic);
	}
	else {
	}
...
EXPORT_SYMBOL(htc_get_wakeup_eint_cnt);
     ...
     for (reg_base = 0; reg_base < EINT_MAX_CHANNEL; reg_base+=32) {
            ...
            for(offset = 0; offset < 32; offset++){
                ...
                if (status_check) {
                    if(index == 212){
                        c2k++;
                        pr_crit("EINT 212 is pending, total %d\n", c2k);
                    }
                    else if(index == 206){
                        pmic++;
                        pr_crit("EINT 206 is pending, total %d\n", pmic);
                    }
                    else
                        pr_crit("EINT %d is pending\n",index);
                }
            }
     }
     printk(KERN_DEBUG"\n");
