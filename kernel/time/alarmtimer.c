...
static int find_offmode_alarm(void)
{
	...
	pr_info("Shutdown time: %lu\n", rtc_now.tv_sec);
        for (i = 0; i < offalarm_size; i++) {
		pr_info("offmode alarm data [%d]: %d\n",i , offalarm[i]);
        ...
        for (i = 0; i < offalarm_snooze_size; i++) {
                pr_info("offmode alarm snooze data [%d]: %d\n",i , offalarm_snooze[i]);
        ...
        alarm_dbg(INFO, "alarm(%u), snooze(%u)", nearest_alarm, nearest_alarm_snooze);
...
{
  alarm_dbg(INFO, "******** alarmtimer: alarm_shutdown ********\n");
	int offmode_alarm, pwron_time = 0;
	...
	if (offmode_alarm > 0) {
 ...
 #ifdef RTC_PWRON_SEC
         /* round down the second */
          offmode_alarm = (offmode_alarm / 60) * 60;
 #endif
         if (offmode_alarm > 0) {
                 pwron_time = offmode_alarm;
 #ifdef RTC_PWRON_SEC
                 pwron_time += RTC_PWRON_SEC;
 #endif
                 rtc_alarm.enabled = 2;
	 ...
	 pr_info("Set offmode alarm power_on time : %u\n", pwron_time);

         alarm_rtc_dev = alarmtimer_get_rtcdev();
         rtc_time_to_tm(pwron_time, &rtc_alarm.time);
         rtc_timer_cancel(alarm_rtc_dev, &rtctimer);
