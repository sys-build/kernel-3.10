...
#define PERF_LEVEL_HIGHEST 5
static int cpufreq_min;
static int cpufreq_max;
		...
		char *buf)
{
	...
	for (i = PERF_LEVEL_HIGHEST-1 ; i >= 0 ; i--) {
		if (cpufreq_min & (1 << i))
		    break;
	}
	if (i < 0)
		...
		const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &val) > 0) {
		bit = val / 2;
		on = val % 2;
		if (bit >= PERF_LEVEL_HIGHEST || bit < 0)
		    return -EINVAL;
		if (on)
		    cpufreq_min |= (1 << bit);
		else
		    cpufreq_min &= ~(1 << bit);
		sysfs_notify(kobj, NULL, "perflock");
		return n;
	}
	return -EINVAL;
}
		...
		char *buf)
{
	...
	for (i = 0 ; i < PERF_LEVEL_HIGHEST ; i++) {
		if (cpufreq_max & (1 << i))
		    break;
	}
	if (i >= PERF_LEVEL_HIGHEST)
	    i = 0;
	else
	    i++;
		...
		const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &val) > 0) {
		...
		if (on)
		    ...
		    cpufreq_max &= ~(1 << bit);
		sysfs_notify(kobj, NULL, "cpufreq_ceiling");
		return n;
	}
	return -EINVAL;
}
		...
		const char *buf, size_t n)
{
	...
	if (sscanf(buf, "%d", &val) > 0) {
		bit = val / 2;
		on = val % 2;
		if (bit >= num_possible_cpus())
			bit = num_possible_cpus() - 1;
		if (bit < 0)
		    return -EINVAL;
		if (on)
		    cpunum_min |= (1 << bit);
