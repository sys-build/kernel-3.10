...
$(INSTALLED_KERNEL_TARGET): $(BUILT_KERNEL_TARGET) $(KERNEL_HEADERS_INSTALL) | $(ACP)
	$(copy-file-to-target)

	$(info start1 build keydar_build_kernel_modules.sh)
	vendor/mocana/scripts/keydar_build_kernel_modules.sh -v -M -c $(KERNEL_CROSS_COMPILE) -s `pwd`/vendor/mocana/src/mss -k $(LINUX_KERNEL_VERSION) -K $(KERNEL_OUT) -e `pwd`/vendor/mocana/src/ecryptfs-mocana -a `pwd`/vendor/mocana/src/crypto-api-template -D `pwd`/$(TARGET_OUT)/lib/modules

ifeq ($(MOCANA_FIPS_MODULE), true)
	$(info start2 build keydar_build_kernel_modules.sh)
	vendor/mocana/scripts/keydar_build_kernel_modules.sh -v -M -c $(KERNEL_CROSS_COMPILE) -s `pwd`/vendor/mocana/src/mss -k $(LINUX_KERNEL_VERSION) -K $(KERNEL_OUT) -r `pwd`/vendor/mocana/src/dm-crypt-mocana -a `pwd`/vendor/mocana/src/crypto-api-template -D `pwd`/$(TARGET_OUT)/lib/modules
endif
...
$(KERNEL_HEADERS_INSTALL): $(TARGET_KERNEL_CONFIG) | $(KERNEL_OUT)
	    $(MAKE) -C $(KERNEL_DIR) O=$(KERNEL_OUT) ARCH=$(TARGET_ARCH) TARGET_PRODUCT=${TARGET_PRODUCT} PRIVATE_RCMS_SKU="${PRIVATE_RCMS_SKU}" MTK_TARGET_PROJECT=${MTK_TARGET_PROJECT} CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) ROOTDIR=$(ROOTDIR) headers_install

.PHONY: kernel install-kernel savedefconfig-kernel %config-kernel clean-kernel
kernel: $(INSTALLED_KERNEL_TARGET) $(KERNEL_HEADERS_INSTALL)
